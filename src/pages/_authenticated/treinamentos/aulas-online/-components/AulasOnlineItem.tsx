    ...
    <>
      ...
      >
        ...
        <div className="flex flex-col lg:w-2/12 relative z-10">
          ...
          <p className="text-sm text-muted-foreground">{item?.course?.name || 'não associado'}</p>
        </div>

        {/* Configuração de Progresso */}
        <div className="flex flex-col lg:w-3/12 relative z-10">
          <p className="text-xs text-muted-foreground lg:hidden">Configuração</p>
          <p className="text-xs">{getProgressConfigDisplay()}</p>
          {item.progressConfig && (
            <div className="flex gap-2 mt-1">
              {item.progressConfig.requireSequential && (
                <span className="text-xs px-1.5 py-0.5 bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 rounded">Sequencial</span>
              )}
              {!item.progressConfig.allowSkip && (
                <span className="text-xs px-1.5 py-0.5 bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400 rounded">Sem pular</span>
              )}
            </div>
          )}
        </div>
