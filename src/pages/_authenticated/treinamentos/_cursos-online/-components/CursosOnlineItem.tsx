...
const Item = ({
  ...
  const getLessonsCount = () => {
    ...
    return `${item.lessons.length} ${item.lessons.length === 1 ? "aula" : "aulas"}`;
  };
    ...
    <>
      ...
      >
        ...
        <div className="flex flex-col lg:w-2/12 relative z-10">
          <p className="text-xs text-muted-foreground lg:hidden">Aulas</p>
          <p className="text-sm">{getLessonsCount()}</p>
          {item.lessons && item.lessons.length > 0 && (
            <p className="text-xs text-muted-foreground">
              {item.lessons.filter((l) => l?.isActive).length} ativas
            </p>
          )}
        </div>
