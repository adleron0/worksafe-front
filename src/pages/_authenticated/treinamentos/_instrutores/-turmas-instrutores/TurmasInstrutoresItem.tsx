...
const CustomerItem = ({ item, index, entity,
  ...
  const { mutate: deactivate } = useMutation({
    ...
    onSuccess: () => {
      hideLoader();
      toast({
        title: `${entity.name} ${item.instructor?.name} inativado!`,
  ...
  const { mutate: activate } = useMutation({
    ...
    onSuccess: () => {
      hideLoader();
      toast({
        title: `${entity.name} ${item.instructor?.name} reativado!`,
    ...
    <>
      ...
      {index === 0 && (
        <div className="hidden lg:flex items-center justify-between py-2 px-4 w-full bg-primary rounded-t-lg font-semibold text-sm text-inverse-foreground">
          <div className="w-4/12">Instrutor</div>
      ...
      <div className={`${index % 2 === 0 ? "bg-background" : "bg-background/50"} relative shadow-sm rounded gap-2 lg:gap-0 flex flex-col lg:flex-row lg:items-center justify-between p-4 w-full border-b`}>
        ...
        <div className="w-full lg:w-4/12 flex items-center space-x-4 md:pr-2">
          <Avatar className="border rounded-md">
            <AvatarImage src={item.instructor?.imageUrl || undefined} alt={item.instructor?.name} />
            <AvatarFallback className="rounded-md">{item.instructor?.name[0]}</AvatarFallback>
          </Avatar>
          <div className="break-words w-9/12 md:w-full">
            <h2 className="text-sm font-semibold">{item.instructor?.name}</h2>
                ...
                !item.inactiveAt ? (
                  can(`inactive_${entity.ability}`) && (
                      <DropdownMenuItem className="p-0" onSelect={(e) => e.preventDefault()}>
                        <ConfirmDialog
                          title={`Inativar o ${entity.name} ${item.instructor?.name}?`}
                          description={`Ao prosseguir, o ${entity.name} ${item.instructor?.name} será desvinculado da turma.`}
                  ...
                  can(`activate_${entity.ability}`) && (	
                    <DropdownMenuItem className="p-0" onSelect={(e) => e.preventDefault()}>
                      <ConfirmDialog
                        title={`Reativar o ${entity.name} ${item.instructor?.name}?`}
                        description={`Ao prosseguir, o ${entity.name} ${item.instructor?.name} será reativado na turma.`}
