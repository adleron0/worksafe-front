...
const CourseLessonsManager: React.FC<CourseLessonsManagerProps> = ({ 
  ...
  const { data: lessonsData, isLoading: isLoadingLessons } = useQuery<ApiResponse>({
  ...
  const { data: linkedLessonsData, isLoading: isLoadingLinked, refetch: refetchLinked } = useQuery<ApiResponse>({
  ...
  const { mutate: updateModelLesson } = useMutation({
    mutationFn: async (data: { modelId: number; lessonId: number; order: number; isActive: boolean }) => {
  ...
  useEffect(() => {
    if (linkedLessonsData?.rows) {
      const active = linkedLessonsData.rows.filter((ml: IModelLesson) => ml.isActive);
      setModelLessons(active.sort((a: IModelLesson, b: IModelLesson) => a.order - b.order));
    }
  }, [linkedLessonsData]);
  ...
  const handleToggleLesson = (lesson: ILesson) => {
    ...
    const isCurrentlyLinked = modelLessons.some(ml => ml.lessonId === lesson.id);
    ...
    if (isCurrentlyLinked) {
      ...
      // Vincular
      const order = modelLessons.length === 0 
        ? 100000 
            ...
            <ScrollArea className="h-[500px] pr-4">
              {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center h-40 text-center">
                  <Icon name="folder-open" className="h-12 w-12 text-muted-foreground mb-2" />
            ...
            <ScrollArea className="h-[500px] pr-4">
              {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center h-40 text-center border-2 border-dashed border-muted-foreground/25 rounded-lg">
                  <Icon name="layers" className="h-12 w-12 text-muted-foreground mb-2" />
                  <p className="text-muted-foreground text-sm">
                ...
                >
                  ...
                  >
                    <div className="space-y-3">
                      {modelLessons.map((modelLesson, index) => (
                        modelLesson.lesson && (
                          <SortableLesson
                            key={modelLesson.lessonId}
                            lesson={modelLesson.lesson}
                            order={index + 1}
                            onRemove={() => handleToggleLesson(modelLesson.lesson!)}
                    ...
                    {activeLesson && (
                      <Card className="shadow-xl border-primary opacity-90">
                        <CardContent className="p-4">
                          <div className="flex items-center gap-2">
                            <Icon name="grip-vertical" className="h-4 w-4 text-muted-foreground" />
                            <h4 className="font-medium text-sm">{activeLesson.title}</h4>
                          </div>
                        </CardContent>
                      </Card>
                    )}
